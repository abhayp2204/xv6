...
scheduler(void)
{
  ...
  int max_priority;
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      ...
      int y = p->static_priority - p->niceness + 5;
      int x = y < 100 ? y : 100;
      p->dynamic_priority = x > 0 ? x : 0;
      if (p->state == RUNNABLE && p->dynamic_priority < max_priority) {
        chosen = p;
    ...
    if (chosen != 0) {
      for (p = proc; p < &proc[NPROC]; p++) {
        ...
        if (p->state == RUNNABLE && p->dynamic_priority == chosen->dynamic_priority && p->no_of_proc < chosen->no_of_proc)
          chosen = p;
        
      }
      
      for (p = proc; p < &proc[NPROC]; p++) {
        if (p->state == RUNNABLE && p->dynamic_priority == chosen->dynamic_priority && p->no_of_proc == chosen->no_of_proc && p->ctime < chosen->ctime) 
          chosen = p;
      }
